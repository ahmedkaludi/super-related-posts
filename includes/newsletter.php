...
class suprp_pointers {
	...
	function admin_enqueue_scripts () {
		...
		$do_tour = !in_array ('suprp_subscribe_pointer', $dismissed);
		if ($do_tour) {
	...
	function admin_print_footer_scripts () {
		...
		if ($show_pointer) {
			//var_dump($id);die;
			$this->suprp_pointer_script ($id, $options, esc_html__('No Thanks', 'super_related_posts'), $button2, $function);
		}
	}
	...
	function suprp_pointer_script ($id, $options, $button1, $button2=false, $function='') {
		?>
		<script type="text/javascript">
			(function ($) {
				...
				wp_pointers_tour_opts = $.extend (wp_pointers_tour_opts, {
					...
					close: function () {
						$.post (ajaxurl, {
							pointer: 'suprp_subscribe_pointer',
...
$suprp_pointers = new suprp_pointers();
?>